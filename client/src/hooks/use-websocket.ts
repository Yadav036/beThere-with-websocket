    ...
    try {
      ...
      ws.onopen = () => {
        ...
        pingIntervalRef.current = setInterval(() => {
          if (ws.readyState === WebSocket.OPEN) ws.send(JSON.stringify({ type: 'ping', timestamp: new Date().toISOString() }));
        }, 30000);
      };
      ...
      ws.onclose = (event) => {
        ...
        if (!manualDisconnectRef.current && reconnectCountRef.current < reconnectAttempts) {
          ...
          reconnectTimeoutRef.current = setTimeout(connect, reconnectDelay * Math.pow(1.5, reconnectCountRef.current - 1));
        }
      };
