...
import { calculateDistance, calculateETA } from "../client/src/lib/haversine";
...
// ❌ FIXED: Enhanced WebSocket authentication with better error handling
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // ❌ FIXED: Enhanced WebSocket connection handling with better logging
  ...
  try {
    ...
    // Handle ping/pong messages without Zod validation
    ...
    // For all other messages, use Zod validation
    ...
    // ❌ FIXED: Better close event handling with detailed logging
    ...
    // ❌ FIXED: Add error event handling
    ws.on('error', (error) => {
...
async function handleWebSocketMessage(
  ...
  switch (message.type) {
    case 'location_update':
      {
        ...
        if (currentParticipant?.lastLat && currentParticipant?.lastLng) {
          ...
          // Check if moved more than 100m in the last update
    ...
    // Remove the pong case since we handle it above
  }
}
...
app.post('/api/events', authenticateToken, async (req: AuthenticatedRequest, res) => {
  ...
  app.delete('/api/events/:id', authenticateToken, async (req: AuthenticatedRequest, res) => {
    try {
      const deleted = await storage.deleteEvent(req.params.id, req.user!.sub);
      if (!deleted) {
        return res.status(403).json({ error: "Not authorized to delete this event" });
      }

      // Broadcast event deletion to all participants
